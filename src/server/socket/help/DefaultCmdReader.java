 ...
 *
 */
public class DefaultCmdReader implements ICmdReader{
	...
	public CmdInfo[] readCmd(ConnectSocketInfo connInfo) throws Exception{
		...
		while(true){
			...
			if(head.remaining() == 0){
				...
				cmdInfo = new CmdInfo();
				cmdInfo.setInfo(CmdInfo.CMD_FLAG, (headByteArr[0]&0xFF)+"");
				...
				if(dataLen == 0){
					cmdInfo = new CmdInfo();
					cmdInfo.setInfo(CmdInfo.CMD_FLAG, (head.get(0)&0xFF)+"");
					cmdInfo.setInfo(CmdInfo.SOCKET_FLAG, connInfo);
