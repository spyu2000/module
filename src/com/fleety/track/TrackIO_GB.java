...
//	该bit=1:表示视频丢失,=0:表示视频正常
	...
	private void writeTrackRecordByVersionIClound(OutputStream out,InfoContainer[] recordInfos) throws Exception{
		...
		ByteBuffer buff = ByteBuffer.allocate((39+8) * recordInfos.length);
		int num = recordInfos.length;
		InfoContainer recordInfo;
		Object temp = null;
		...
		for (int i = 0; i < num; i++) {
			recordInfo = recordInfos[i];
			temp=recordInfo.getInfo(TrackIO_GB.DEST_LO_FLAG);
			if(temp!=null){
				buff.putDouble(Double.parseDouble(temp.toString()));
			}else{
				buff.putDouble(0);
			}	
			temp=recordInfo.getInfo(TrackIO_GB.DEST_LA_FLAG);
			if(temp!=null){
				buff.putDouble(Double.parseDouble(temp.toString()));
			}else{
				buff.putDouble(0);
			}
			temp=recordInfo.getInfo(TrackIO_GB.DEST_TIME_FLAG);
			if(temp!=null){
				buff.putLong(((Date)temp).getTime());
			}else{
				buff.putLong(0);
			}			
			temp=recordInfo.getInfo(TrackIO_GB.DEST_STATUS_FLAG);
			if(temp!=null){
				status = Integer.parseInt(temp.toString());
			}else{
				status=0;
			}			
			temp=recordInfo.getInfo(TrackIO_GB.DEST_ALARM_TYPE_FLAG);
			if(temp!=null){
			...
			temp=recordInfo.getInfo(TrackIO_GB.DEST_SPEED_FLAG);
			if(temp!=null){
				buff.putShort((short)Integer.parseInt(temp.toString()));
			}else{
				buff.putShort((short)0);
			}		
			temp=recordInfo.getInfo(TrackIO_GB.DEST_DIRECTION_FLAG);
			if(temp!=null){
				buff.putShort((short)Integer.parseInt(temp.toString()));
			}else{
				buff.putShort((short)0);
			}			
			temp=recordInfo.getInfo(TrackIO_GB.DEST_KILO_FLAG);
			if(temp!=null){
				buff.putInt(Integer.parseInt(temp.toString()));
			}else{
				buff.putInt(0);
			}			
			temp = recordInfo.getInfo(TrackIO_GB.DEST_OIL_FLAG);
			if(temp!=null){
				buff.putShort((short)Integer.parseInt(temp.toString()));
			}else{
				buff.putShort((short)0);
			}			
			temp = recordInfo.getInfo(TrackIO_GB.DEST_TEMP1_FLAG);
			if (temp != null) {
				buff.putShort(((Integer) temp).shortValue());
			} else {
				buff.putShort((short) -12800);
			}
			temp = recordInfo.getInfo(TrackIO_GB.DEST_TEMP2_FLAG);
			if (temp != null) {
				buff.putShort(((Integer) temp).shortValue());
			} else {
				buff.putShort((short) -12800);
			}
			temp = recordInfo.getInfo(TrackIO_GB.DEST_ALL_STATUS_FLAG);
			if (temp != null) {
			...
			temp = recordInfo.getInfo(TrackIO_GB.DEST_EVENT_ID_FLAG);
			if (temp != null) {
				buff.putInt(((Integer) temp).intValue());
			} else{
				buff.putInt(0);
	...
	private int OLD_RECORD_LENGTH = 39+8;
...
//		TrackIO.addFloat(buff, info, DEST_LA_FLAG);
		...
		TrackIO_GB.addDouble(buff, info, DEST_LO_FLAG);
//		System.out.println("lo:"+((Double)info.getInfo(DEST_LO_FLAG)).doubleValue());
		TrackIO_GB.addDouble(buff, info, DEST_LA_FLAG);
		TrackIO_GB.addDate(buff, info, DEST_TIME_FLAG);
		...
		TrackIO_GB.addUnshort(buff, info, DEST_SPEED_FLAG);
		TrackIO_GB.addUnshort(buff, info, DEST_DIRECTION_FLAG);
		
		if(this.OLD_RECORD_LENGTH == 39+8){
			if(this.OLD_RECORD_VERSION.equals(TrackIO_GB.VERSION_ICLOUD_FLAG)){
				//大平台轨迹文件中油量在温度之前
				TrackIO_GB.addInt(buff, info, DEST_KILO_FLAG);
				TrackIO_GB.addShort(buff, info, DEST_OIL_FLAG);
				
				TrackIO_GB.addShort(buff, info, DEST_TEMP1_FLAG);
				TrackIO_GB.addShort(buff, info, DEST_TEMP2_FLAG);
				TrackIO_GB.addInt(buff, info, DEST_ALL_STATUS_FLAG);
				TrackIO_GB.addInt(buff, info, DEST_EVENT_ID_FLAG);
			}else{
				//iflow历史轨迹中油量在最后一个位置
				TrackIO_GB.addInt(buff, info, DEST_KILO_FLAG);				
				TrackIO_GB.addShort(buff, info, DEST_TEMP1_FLAG);
				TrackIO_GB.addShort(buff, info, DEST_TEMP2_FLAG);
				TrackIO_GB.addShort(buff, info, DEST_TEMP3_FLAG);
				TrackIO_GB.addShort(buff, info, DEST_TEMP4_FLAG);
				TrackIO_GB.addShort(buff, info, DEST_TEMP5_FLAG);
				TrackIO_GB.addShort(buff, info, DEST_TEMP6_FLAG);				
				TrackIO_GB.addShort(buff, info, DEST_OIL_FLAG);
			}
			
		}else if(this.OLD_RECORD_LENGTH == 22){
			TrackIO_GB.addByte(buff, info, DEST_LOCATE_FLAG);
	...
	private InfoContainer readTrackRecordByVersion1(InputStream in) throws Exception{
		...
		for(int i=0;i<headNum;i++){
			...
			if(curHead.equals(DEST_LO_FLAG)){
				...
				TrackIO_GB.addByteArr(data,data.get()&0xFF,info,curHead);
			}else if(curHead.equals(DRIVER_NO_INFO_FLAG)){
				TrackIO_GB.addString(data,data.get()&0xFF,info,curHead);
	...
	private static final void addDouble(ByteBuffer buff,InfoContainer info,Object key){
		double f = buff.getDouble();
		info.setInfo(key, new Double(f));
	}
